...
root_agent = LlmAgent(
   ...
   description="AI orchestration agent that can find nearest plumbers and electricians using Google Places API, search for material prices using Groq API, estimate construction materials from floorplan images, generate 3D STL models from 2D floorplans, and create HVAC and electrical layout drawings",
   # Instructions to set the agent's behavior.
   instruction=instruction,
   tools=[get_nearest_trades, search_for_materials, get_materials_from_floorplan, generate_3d_model_from_floorplan, get_hvac_and_electrical_drawings]
)